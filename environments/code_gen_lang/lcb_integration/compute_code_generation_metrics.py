...
@ray.remote(
    ...
    # The sandbox itself is robust (reproduced sequential/concurrent/pathological, 0 raises);
    # the FileNotFoundError that scores ~38% of code rollouts a false 0 originates in Ray's
    # per-task runtime_env / worker-CWD setup under sustained multi-node SPREAD load, NOT in
    # check_correctness. A genuine wrong answer *returns* [-1]/False and never raises, so it is
    # always safe to retry a raise: Ray reschedules on a (possibly fresh) worker, recovering the
    # flake instead of poisoning the reward signal. max_retries is the retry budget; retry_exceptions
    # opts the FileNotFoundError application error into that budget (default is no app-error retry).
    max_retries=3,
    retry_exceptions=[FileNotFoundError],
)
