#!/usr/bin/env python3
"""Prepare the multilingual code_gen_lang training data.

Like the stem_mcqa blend, the competitive-coding blend already ships each row in
Gym shape: the per-language prompt is prebuilt in responses_create_params.input,
and verifier_metadata.unit_tests (LiveCodeBench inputs/outputs) + language +
pass_rate are top-level. So this script only:
  - filters by profiled pass_rate (drop ONLY the utterly-hopeless rows; ~40% are
    pass_rate==0 here, unsolvable even by the stronger NVIDIA profiler), keeping
    everything it solved at least once -- our own 30B profiling is the real filter,
  - drops en-labelled rows whose prompt is actually Chinese (failed translation),
  - retargets agent_ref -> code_gen_lang_simple_agent,
  - splits into train / validation / validation_smoke, stratified by language.

Pure stdlib. Run on the login node:  python3 prepare_code_gen_lang_data.py
"""
import argparse
import collections
import copy
...
SRC = "${BLEND_SRC}/data/translated/blended/nano_v3_sft_profiled_comp_coding_50tests.jsonl"
OUT_DIR = "data"
ENV_DATA_DIR = "./data"
AGENT_NAME = "code_gen_lang_simple_agent"

# CJK Unified Ideographs (incl. Ext-A); used to drop en rows that are really
# Chinese (same filter as prepare_stem_mcqa_data.py). yue/zh-hk are exempt.
...
def prompt_text(row: dict) -> str:
    """Concatenate the user-turn content(s) of the prebuilt prompt."""
    parts = []
