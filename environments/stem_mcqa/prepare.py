#!/usr/bin/env python3
"""Prepare the multilingual stem_mcqa training data.

Unlike the math env, the MCQA blend already ships each row in Gym shape: the
per-language prompt is prebuilt in responses_create_params.input, and the grading
fields (options / expected_answer / template_metadata.output_regex / language) are
top-level. So this script only:
  ...
  - retargets agent_ref -> stem_mcqa_simple_agent,
  - splits into train / validation / validation_smoke, stratified by language.

Pure stdlib. Run on the login node:  python3 prepare_stem_mcqa_data.py
"""
import argparse
import collections
import copy
...
SRC = "${BLEND_SRC}/data/translated/blended/nano_v3_sft_profiled_stem_mcqa.jsonl"
OUT_DIR = "data"
ENV_DATA_DIR = "./data"
AGENT_NAME = "stem_mcqa_simple_agent"

# CJK Unified Ideographs (incl. Ext-A); used to drop en rows that are really
# Chinese (same filter as prepare_math_lang_data.py). yue/zh-hk are exempt.
